<nav class="w-full px-6 flex items-center justify-between h-16">
    ...
    <a href="{{ url('/') }}">
        <div class="flex items-center gap-3">
            <img src="{{ asset('images/LOGO-BSL-.png') }}" onerror="this.style.display='none'" class="h-10 w-auto"
                alt="Logo" />
            <div>
    ...
    <div class="hidden md:flex gap-8 text-sm text-gray-300 font-light">
        ...
        @if (Auth::check() && Auth::user()->hasRole('customer'))
            <a href="{{ route('pesanan.index') }}" class="hover:text-white transition">Pesanan</a>
        @endif
    </div>

    <!-- Right Side -->
    <div class="flex items-center gap-3">
        @unless(Route::currentRouteNamed('checkout.index', 'pesanan.index', 'pesanan.show'))
